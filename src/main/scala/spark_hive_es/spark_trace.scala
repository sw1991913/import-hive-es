...
import Constants.{parameterLog, parameterTrace}
import MyLog.MyDfsLog
import Util.aboutHive._
import Util.aboutHive3
...
object spark_trace {
    ...
    //提取输入参数
      ...
      * */
      ...
      MyDfsLog.AddLog("/spark/task.log",jobName,sc.applicationId+","+starttime+","+endtime)

      // 获取 confFile的 导入类型
      val importType=getValue(confFile,"spark.hive.es.import.type").trim
      if (importType =="")
      {
        throw new Exception("spark.hive.es.import.type 不能为空")
      }
      val tmpDirPathName=getValue(confFile,"spark.es.output.tmpdir").trim
      if (tmpDirPathName =="")
      {
        throw new Exception("spark.es.output.tmpdir 不能为空")
      }

//****先清空临时目录*********
      println("tmpDir==>"+tmpDirPathName)
      deleteTmpDir(new Path(tmpDirPathName),fs)
      ...
      //  println("log条数"+rowRDD.count)
        val isSuccess= HiveWriteHDFS2(rowRDD,Constant.tmpDir,starttime,mysql:MysqlUtil,jobName,sc,Constant.fileprefix,endtime)
// **************************
        if (isSuccess){
          reNameFile(Constant.tmpDir,Constant.tolibDir,fs)
        }
...
//***************************
        ...
        val RDD=  if (Constant.source.equalsIgnoreCase("2")){  // 审计 termlog
        ...
        val rowRDD=RDD.map(x=>(x.getString(0).replaceAll("\"",""),(x.getLong(1),x.getString(2).replaceAll("\"",""))))
           ...
           && isNumber(x._2._1.toString)))
        println("source  => "+Constant.source+",day  =>  "+timeStamp2Day(starttime))

        val resultRDD=aboutHive3.handlerTrace3(rowRDD).map {
          x=> (x._2.toString,x._1+"\t"+x._2+"\t"+x._3+"\t"+x._4+"\t"+Constant.source)}.coalesce(Constant.numpartitions,false)
       // println("trace条数"+resultRDD.count())
        // 写到 临时目录

        val isSuccess=HiveWriteHDFS2(resultRDD,Constant.tmpDir,starttime,mysql:MysqlUtil,jobName,sc,Constant.fileprefix,endtime)
//***************************
        // 临时目录 => 入库目录
        if (isSuccess){
          reNameFile(Constant.tmpDir,Constant.tolibDir,fs)
